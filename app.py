...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:4Rocks32@localhost/rachjrHW4db"  # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('Search_Gifs', db.Column('search_id', db.Integer, db.ForeignKey('search.id')), db.Column('gif_id', db.Integer, db.ForeignKey('gif.id')))
...
class User(UserMixin, db.Model):
    ...
    collection_rel = db.relationship('PersonalGifCollection',backref='User')
...
class PersonalGifCollection(db.Model):
    ...
    many_gif_col = db.relationship('Gif', secondary=user_collection, backref=db.backref('collection', lazy='dynamic'), lazy='dynamic') 
...
class SearchTerm(db.Model):
    ...
    many_gif_s = db.relationship('Gif', secondary=search_gifs, backref=db.backref('search', lazy='dynamic'), lazy='dynamic') 
    def __repr__(self):
...
def get_gifs_from_giphy(search_string):
    ...
    url = baseurl + "?q=" + str(search_string) + '&api_key=' + api_key + "&limit=5"
    ...
    # Then the function shoul d process the response in order to return a list of 5 gif dictionaries.
...
def get_or_create_search_term(term):
    ...
    if searchTerm:
        ...
        gif_info = get_gifs_from_giphy(search_string=term)
        a = list()
        print(searchTerm)
        for g in gif_info:
            x = get_or_create_gif(title=g['title'], url=g['embed_url'])
            searchTerm.many_gif_s.append(x)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    pc = db.session.query(PersonalGifCollection).filter_by(name=name,user_id=current_user.id).first()
    if pc:
        return pc
    else:
        pc = PersonalGifCollection(name=name,user_id=current_user.id,many_gif_col=[])
        for gl in gif_list:
            pc.many_gif_col.append(gl)
        db.session.add(pc)
        db.session.commit()
    return pc
...
def index():
    form = GifSearchForm()
    if form.validate_on_submit():
        search_term = str(form.search.data)
...
def search_results(search_term):
    ...
    relevant_gifs = term.many_gif_s.all()
    return render_template('searched_gifs.html',term=term,gifs=relevant_gifs)

@app.route('/search_terms')
def search_terms():
    all_searched = []
    searched = SearchTerm.query.all()
    return render_template('search_terms.html', all_terms = searched)
...
@login_required
def collections():
    current_user_collection = db.session.query(PersonalGifCollection).filter_by(user_id=current_user.id)
    return render_template('collections.html', collections=current_user_collection)
...
def single_collection(id_num):
    ...
    gifs = collection.many_gif_col.all()
